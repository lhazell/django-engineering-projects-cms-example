...
class ReadonlyJobAdmin(jobAdmin):
    def __init__(self, model, admin_site):
      super(ReadonlyJobAdmin, self).__init__(model, admin_site)
      self.model = model
    ...
    def get_actions(self, request):
        actions = super(ReadonlyJobAdmin, self).get_actions(request)
        if request.user.has_perm('project.readonly') and not request.user.is_superuser:
...
admin.site.register(Job, ReadonlyJobAdmin)